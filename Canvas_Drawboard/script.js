    ...
    let width = 10
    let radius = function () {return width/2}

    let color = 'black'
    //生成 颜色按钮 和 尺寸按钮 的列表
    let colorsNode = document.getElementById('colors')
    let colorButtons = get_children(colorsNode)
    let sizesNode = document.getElementById('sizes')
    let sizeButtons = get_children(sizesNode)
